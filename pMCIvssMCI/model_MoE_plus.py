...
class feature_Netin_tp4_multr_score_dualBranch(nn.Module):
	...
	def forward(self, x):
		...
		if self.use_moe == "MoE":
			...
			ensemble_logits = torch.sigmoid(self.fusion_alpha) * fused_logit + (1 - torch.sigmoid(self.fusion_alpha)) * cls_branch_logits
			return fused_logit, cls_branch_logits, ensemble_logits, logits1, logits2, logits3, logits4, gate_weights, top_k_indices, diversity_metrics
		elif self.use_moe == "concat":
			reg_features = [features_x1, features_x2, features_x3, features_x4]
			features = torch.cat(reg_features, dim=1)
			diversity_metrics = self._compute_diversity_metrics(reg_features)
