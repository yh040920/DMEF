...
def extract_features_from_model(extractor, data_dir, device, class_order=['SMCI', 'PMCI']):
    features_list = []
    labels_list = []
    sample_names = []
...
def plot_correlation_matrix(corr_matrix, labels, model_name, save_path):
    ...
    n_smci = np.sum(labels == 0)
    n_pmci = np.sum(labels == 1)
    ...
    if n_smci > 0 and n_pmci > 0:
        ax.axhline(y=n_smci, color='black', linewidth=2)
        ax.axvline(x=n_smci, color='black', linewidth=2)
        
        ax.text(n_smci/2, -5, 'SMCI', ha='center', fontsize=14, fontweight='bold')
        ax.text(n_smci + n_pmci/2, -5, 'PMCI', ha='center', fontsize=14, fontweight='bold')
        ax.text(-5, n_smci/2, 'SMCI', ha='center', va='center', fontsize=14, fontweight='bold', rotation=90)
        ax.text(-5, n_smci + n_pmci/2, 'PMCI', ha='center', va='center', fontsize=14, fontweight='bold', rotation=90)
    
    ax.set_title(f'Sample-wise Cosine Similarity Matrix - {model_name}\n(SMCI group first, then PMCI group.)', 
...
def plot_all_models_combined(all_corr_matrices, all_labels, model_names, save_path):
    ...
    for idx, (model_name, corr_matrix) in enumerate(zip(model_names, [all_corr_matrices[m] for m in model_names])):
        ...
        if n_smci > 0 and n_pmci > 0:
            ax.axhline(y=n_smci, color='black', linewidth=1.5)
            ax.axvline(x=n_smci, color='black', linewidth=1.5)
        
        ax.set_title(f'{model_name}', fontsize=14, fontweight='bold')
        ax.set_xlabel('')
        ax.set_ylabel('')
    
    fig.suptitle('Sample-wise Cosine Similarity Matrices (SMCI group first, then PMCI group.)', 
...
def compute_intra_inter_class_correlation(corr_matrix, labels):
    ...
    smci_indices = np.where(labels == 0)[0]
    pmci_indices = np.where(labels == 1)[0]
    
    intra_smci_corrs = []
    for i in range(len(smci_indices)):
        for j in range(i+1, len(smci_indices)):
            intra_smci_corrs.append(corr_matrix[smci_indices[i], smci_indices[j]])
    
    intra_pmci_corrs = []
    for i in range(len(pmci_indices)):
        for j in range(i+1, len(pmci_indices)):
            intra_pmci_corrs.append(corr_matrix[pmci_indices[i], pmci_indices[j]])
    
    inter_corrs = []
    for i in smci_indices:
        for j in pmci_indices:
            inter_corrs.append(corr_matrix[i, j])
    
    results = {
        'intra_SMCI_mean': np.mean(intra_smci_corrs) if intra_smci_corrs else 0,
        'intra_SMCI_std': np.std(intra_smci_corrs) if intra_smci_corrs else 0,
        'intra_PMCI_mean': np.mean(intra_pmci_corrs) if intra_pmci_corrs else 0,
        'intra_PMCI_std': np.std(intra_pmci_corrs) if intra_pmci_corrs else 0,
...
def main():
    ...
    save_dir = r"/3251903008/yyh/pMCIsMCI/feature_sim_analysis"
    os.makedirs(save_dir, exist_ok=True)
    
    model_configs = {
        'ACNN': {
            'path': r"/3251903008/yyh/result_MoE_SMCI&PMCI/ACNN_ablation_20260628_112038_data_dataArgument/models/3DNet4l_1_ACNN_epoch_90_d1_det.pt",
            'type': 'baseline',
            'feature_dim': 128
        },
        'CACNN': {
            'path': r"/3251903008/yyh/result_MoE_SMCI&PMCI/concat_finetune_20260625_150137_data_dataArgument_top2_bl0.05_dl0.01_detach/models/3DNet4l_1_tp4_sifeature_clonefs_1bt1_l43_1_epoch_90_d1_det.pt",
            'type': 'moe',
            'use_moe': 'concat',
            'feature_dim': 512
        },
        'OURS-simple': {
            'path': r"/3251903008/yyh/result_MoE_SMCI&PMCI/simpleTA_finetune_20260628_043537_data_dataArgument_top2_bl0.05_dl0.01_detach/models/3DNet4l_1_tp4_sifeature_clonefs_1bt1_l43_1_epoch_42_d1_det.pt",
            'type': 'simpleTA',
            'top_k': 2,
            'feature_dim': 128
        },
        'OURS': {
            'path': r"/3251903008/yyh/result_MoE_SMCI&PMCI/MoE_finetune_20260613_153106_data_dataArgument_top2_bl0.05_dl0.01_detach/models/3DNet4l_1_tp4_sifeature_clonefs_1bt1_l43_1_epoch_58_d1_det.pt",
            'type': 'moe',
            'use_moe': 'MoE',
            'top_k': 2,
            'feature_dim': 128
    ...
    for model_name, config in model_configs.items():
        ...
        print(f"Number of SMCI samples: {np.sum(labels == 0)}")
        print(f"Number of PMCI samples: {np.sum(labels == 1)}")
        ...
        print(f"Intra-SMCI similarity: {corr_results['intra_SMCI_mean']:.4f} ± {corr_results['intra_SMCI_std']:.4f}")
        print(f"Intra-PMCI similarity: {corr_results['intra_PMCI_mean']:.4f} ± {corr_results['intra_PMCI_std']:.4f}")
    ...
    print(f"{'='*60}")
    print(f"{'Model':<15} {'Intra-SMCI':<20} {'Intra-PMCI':<20} {'Inter-class':<20}")
    print("-" * 75)
    for model_name, results in all_corr_results.items():
        intra_smci = f"{results['intra_SMCI_mean']:.4f} ± {results['intra_SMCI_std']:.4f}"
        intra_pmci = f"{results['intra_PMCI_mean']:.4f} ± {results['intra_PMCI_std']:.4f}"
        inter = f"{results['inter_mean']:.4f} ± {results['inter_std']:.4f}"
        print(f"{model_name:<15} {intra_smci:<20} {intra_pmci:<20} {inter:<20}")
