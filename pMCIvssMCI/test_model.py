...
def test_model(test_data, ResNet_3D1, device, model_type):
    """
    测试模型性能
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='/3251903008/yyh/result_MoE_SMCI&PMCI/MoE_finetune_20260704_134108_data_dataArgument_top2_bl0.05_dl0.01_detach/models/3DNet4l_1_tp4_sifeature_clonefs_1bt1_l43_1_epoch_33_d1_det.pt', 
    ...
    print("\n===== 加载数据 =====")
    print("加载训练数据以获取统计量...")
    train_loader, train_stats = load_data(args, args.train_root_path, args.PMCI_dir, args.SMCI_dir, args.excel_file, known_args.norm_type)
    
    # 使用训练集的统计量加载测试数据
    print("加载测试数据...")
    test_loader = load_test_data(args, args.test_root_path, args.PMCI_dir, args.SMCI_dir, args.excel_file, train_stats, known_args.norm_type)
    ...
    ResNet_3D1 = feature_Netin_tp4_multr_score(use_moe=known_args.model_type, top_k=2, dropout=args.dropout).to(device)
    ...
    # 执行测试
    print("\n===== 开始测试 =====")
    ACC, SEN, SPE, AUC = test_model(test_loader, ResNet_3D1, device, known_args.model_type)
    ...
    with open(result_file, "w") as f:
        ...
        f.write(f"模型类型: {known_args.model_type}\n")
