...
import nibabel
import os
from model_MoE_plus import feature_Netin_tp4_multr_score_simpleTA


class VisualizationModel(nn.Module):
    def __init__(self, model_path, device, top_k=2, dropout=0.0):
    ...
    def forward_with_ta_feature(self, x):
        ...
        ta_feature = self.model.de1(features_x, map1) * features_x
        ta_feature.retain_grad()
        self.ta_feature = ta_feature
        
        shared_pooled = self.model.shared_pool(ta_feature)
...
def process_single_image(MRI_path, class_label, data_type, test_model, device, args, save_dir):
    ...
    simpleTA模型：只有一个共享的TA特征图，直接可视化即可
    """
    ...
    output1, logits1, logits2, logits3, logits4, gate_weights, top_k_indices, diversity_metrics = test_model.forward_with_ta_feature(MRI_tensor)
    ...
    print('simpleTA mode: using single shared TA feature map (no fusion needed)')
    ...
    save_path = os.path.join(class_save_dir, f'grad_CAM_{class_label}_{filename}_pred_{pred_class_name}_simpleTA.png')
...
def main():
    ...
    # 模型路径
    model_path = r"/3251903008/yyh/result_MoE_SMCI&PMCI/simpleTA_finetune_20260628_043537_data_dataArgument_top2_bl0.05_dl0.01_detach/models/3DNet4l_1_tp4_sifeature_clonefs_1bt1_l43_1_epoch_42_d1_det.pt"
    ...
    # 保存可视化结果的目录
    save_dir = r"/3251903008/yyh/result_MoE_SMCI&PMCI/simpleTA_finetune_20260628_043537_data_dataArgument_top2_bl0.05_dl0.01_detach"
    ...
    test_model = VisualizationModel(model_path, device, top_k=top_k, dropout=dropout)
