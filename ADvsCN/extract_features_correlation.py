...
def compute_correlation_matrix(features, labels):
    ...
    for i in range(n_samples):
        for j in range(n_samples):
            corr, _ = stats.pearsonr(features[i], features[j])
...
def plot_correlation_matrix(corr_matrix, labels, model_name, save_path):
    ...
    ax.set_title(f'Sample-wise Correlation Matrix - {model_name}\n(AD group first, then NC group.)', 
...
def plot_all_models_combined(all_corr_matrices, all_labels, model_names, save_path):
    ...
    fig.suptitle('Sample-wise Correlation Matrices (AD group first, then NC group.)', 
                 fontsize=16, fontweight='bold', y=1.02)
    
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved combined correlation matrix to: {save_path}")
...
def main():
    ...
    save_dir = r"/3251903008/yyh/imagescore_code/feature_correlation_analysis"
    ...
    for model_name, config in model_configs.items():
        ...
        print(f"Intra-AD correlation: {corr_results['intra_AD_mean']:.4f} ± {corr_results['intra_AD_std']:.4f}")
        print(f"Intra-NC correlation: {corr_results['intra_NC_mean']:.4f} ± {corr_results['intra_NC_std']:.4f}")
        print(f"Inter-class correlation: {corr_results['inter_mean']:.4f} ± {corr_results['inter_std']:.4f}")
        
        save_path = os.path.join(save_dir, f'{model_name}_correlation_matrix.png')
        ...
        np.save(os.path.join(save_dir, f'{model_name}_corr_matrix.npy'), corr_matrix)
        
        del extractor
        torch.cuda.empty_cache()
    
    model_order = ['ACNN', 'CACNN', 'OURS-simple', 'OURS']
    combined_save_path = os.path.join(save_dir, 'all_models_correlation_matrix_combined.png')
